...
class Blue2(BlueBot):
    def __init__(self, room, x, y):
        ...
        try:
            ...
            print("hello this is me making a error checking for the set image we used images in our testing so we actually knew which bot was which if youre seeing this that means we again forgot to remove the set image for blue2 which is awkward gotta say so bye have fun doing the competition.")


    def tick(self):
        # Lame declaring outside init becuz of weird glitch with gameframe
        print(f"corey's dumb Blue bot is in {self.curr_state}")
        print(Globals.blue_bots[1].x, Globals.blue_bots[1].y)
