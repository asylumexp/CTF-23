...
class Red3(RedBot):
    ...
    def tick(self):
        # if self.curr_state == "hello":
        #     print("hello")
        #     print(self.x, self.y)
        #     self.general_stack_drive(self, 600, 250)
    ...
    def return_home(self: RedBot, STATE):
        ...
        if (self.point_to_point_distance(self.x, self.y, self.psuedoflagx, Globals.blue_flag.y) > 40):
            ...
            # if i < 0 or i > 40:
            self.turn_towards(self.psuedoflagx,
    ...
    def bait_bot_bait(self: RedBot, jail_state):
        ...
        # ? move across border, evading enemies
            ...
            self.turn_towards(Globals.red_flag.x,
                              Globals.red_flag.y, Globals.FAST)
            self.drive_forward(Globals.FAST)
        elif self.has_flag:
            i = Globals.red_bots[2].angleRelative(
                self, Globals.red_bots[0].x, Globals.red_bots[0].y)
            if i < 0 or i > 45:
                self.turn_towards(
                    Globals.red_bots[0].x, Globals.red_bots[0].y, Globals.SLOW)
            self.drive_forward(Globals.FAST)
    ...
    def evadeBots(self: RedBot):
        ...
        closest_bot, dist = Globals.red_bots[2].closest_enemy_to_self(
            self, True)
    ...
    def jailbreak(self: RedBot, return_state: STATE):
        ...
        if not bot_jailed:
            ...
            angle = self.angleRelative(
                Globals.SCREEN_WIDTH - 75, Globals.SCREEN_HEIGHT - 100)
            self.turn_towards(Globals.SCREEN_WIDTH - 75,
                              Globals.SCREEN_HEIGHT - 100, Globals.FAST)
            if angle < 120:
                self.drive_forward(Globals.FAST)
